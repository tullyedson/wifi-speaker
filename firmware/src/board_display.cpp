...
void text(const String& value, int y, int size, uint16_t color = foreground) {
    ...
    display.setCursor(std::max(0, (display_panel::width - static_cast<int>(value.length()) * 6 * size) / 2), y);
    display.print(value);
}
...
void draw_face(uint32_t now, uint16_t color) {
    if (!face.getBuffer()) {
        for (int x : {72, 168}) {
            display.drawLine(left + x - 15, 99, left + x + 15, 129, 0xFA8A);
            display.drawLine(left + x - 15, 129, left + x + 15, 99, 0xFA8A);
        }
        return;
    }
    ...
    display.drawRGBBitmap(left, 37, face.getBuffer(), 240, 166);
}
}

bool available() { return true; }
void begin() {
    touch_ready = display_panel::begin();
...
void update(const String& state, uint8_t volume, float mic_gain, uint16_t level, const String& address, bool presence_available, bool presence_detected) {
    ...
    if (dirty || state != old_state || address != old_address || volume != old_volume || mic_gain != old_gain) {
        ...
        if (!eyes) {
            display.drawCircle(left + 120, 120, 116, 0x2147); text("SMART SPEAKER", 26, 1, accent); text(state, 48, 2, color);
            display.drawRoundRect(left + 49, 77, 142, 10, 4, 0x4208); display.fillRoundRect(left + 62, 94, 116, 23, 8, 0x2147);
    ...
    if (eyes) draw_face(now, status_colors::rgb565(state));
        ...
        if (width != old_level) { display.fillRect(left + 52, 80, 136, 4, background); if (width) display.fillRect(left + 52, 80, width, 4, accent); old_level = width; }
    }
}
}
#else
